UNINATIVE_STAGING_DIR = "${TMPDIR}/sysroots-uninative"
UNINATIVE_LOADER ?= "${UNINATIVE_STAGING_DIR}/${BUILD_ARCH}-linux/lib/${@bb.utils.contains('BUILD_ARCH', 'x86_64', 'ld-linux-x86-64.so.2', 'ld-linux.so.2', d)}"
...
python uninative_event_fetchloader() {
    ...
    try:
        ...
        cmd = d.expand("mkdir -p ${UNINATIVE_STAGING_DIR}; cd ${UNINATIVE_STAGING_DIR}; tar -xjf ${UNINATIVE_DLDIR}/%s/${UNINATIVE_TARBALL}; ${UNINATIVE_STAGING_DIR}/relocate_sdk.py ${UNINATIVE_STAGING_DIR}/${BUILD_ARCH}-linux ${UNINATIVE_LOADER} ${UNINATIVE_LOADER} ${UNINATIVE_STAGING_DIR}/${BUILD_ARCH}-linux/${bindir_native}/patchelf-uninative ${UNINATIVE_STAGING_DIR}/${BUILD_ARCH}-linux${base_libdir_native}/libc*.so" % chksum)
...
def enable_uninative(d):
    ...
    if os.path.exists(loader):
        ...
        d.prependVar("PATH", "${UNINATIVE_STAGING_DIR}/${BUILD_ARCH}-linux${bindir_native}:")
